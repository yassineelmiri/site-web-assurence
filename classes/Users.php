...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','تم حفظ تفاصيل المستخدم بنجاح.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
			...
			if($qry){
				$this->settings->set_flashdata('success','تم تحديث تفاصيل المستخدم بنجاح.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
						if($this->settings->userdata('id') == $id)
	...
	public function delete_customer(){
		...
		if($qry){
			$this->settings->set_flashdata('success','تم حذف تفاصيل العميل بنجاح.');
			$resp['status'] = 'success';
