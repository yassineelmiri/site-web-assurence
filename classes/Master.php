...
Class Master extends DBConnection {
	...
	function save_brand(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "العلامة التجارية موجودة بالفعل.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "تم حفظ العلامة التجارية الجديدة بنجاح.";
			else
				$resp['msg'] = " تم تحديث العلامة التجارية بنجاح.";
		}else{
			$resp['status'] = 'failed';
	...
	function save_car_type(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "نوع السيارة موجود بالفعل.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "تم حفظ نوع السيارة الجديدة بنجاح.";
			else
				$resp['msg'] = " تم تحديث نوع السيارة بنجاح.";
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_car_type(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," تم الحذف بنجاح.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_model(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "تم حفظ النموذج الجديد بنجاح.";
			else
				$resp['msg'] = "تم تحديث النموذج بنجاح.";
		}else{
			$resp['status'] = 'failed';
	...
	function delete_model(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," تم حذف النموذج بنجاح.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_vehicle(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "تم حفظ السيارة الجديدة بنجاح.";
			else
				$resp['msg'] = " تم تحديث السيارة بنجاح.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = $this->conn->error;
			$resp['sql'] = $sql;
	...
	function save_transaction(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success'," تم حفظ المعاملة بنجاح.");
			else
				$this->settings->set_flashdata('success'," تم تحديث المعاملة بنجاح");
	...
	function delete_transaction(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," تم حذف المعاملة بنجاح.");
			if($get->num_rows > 0){
